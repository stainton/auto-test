...
test('POST /v1/planner/estimate validates input and returns the runtime result', async t => {
  const submit = await setup(t, async (input, signal) => {
    ...
    return { suggestedCaseCount: 12, rationale: '覆盖登录主流程与错误密码' };
  });
  ...
  assert.deepEqual(await ok.json(), { suggestedCaseCount: 12, rationale: '覆盖登录主流程与错误密码' });
});
...
test('createEstimator calls a tool-free runtime and rejects an invalid count', async () => {
  let captured;
  const estimate = createEstimator({ runtime: async options => { captured = options; return { suggestedCaseCount: 8, rationale: '理由' }; }, model: 'haiku' });
  const controller = new AbortController();
  assert.deepEqual(await estimate(estimateInput, controller.signal), { suggestedCaseCount: 8, rationale: '理由' });
