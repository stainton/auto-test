...
export async function main() {
  ...
  const productExperience=new ProductExperienceStore(process.env.AUTOMATION_PRODUCT_EXPERIENCE_FILE??path.join(dataDir,'product-exploration-experience.json'));
  let plannerJobs;
  const plannerWorker=withExperience(createPlannerWorker({command,runtime:plannerRuntime,playwrightPackage,assetCache}),experience,productExperience);
  ...
  const generatorWorker=withExperience(createGeneratorWorker({command,runtime:generatorRuntime,playwrightPackage,assetCache,caseTimeoutMs:positive('GENERATOR_CASE_TIMEOUT_MS',1800000)}),experience,productExperience);
