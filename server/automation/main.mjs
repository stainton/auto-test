...
export async function main() {
  ...
  const generatorWorker=withExperience(createGeneratorWorker({command,runtime:generatorRuntime,playwrightPackage,caseTimeoutMs:positive('GENERATOR_CASE_TIMEOUT_MS',600000)}),experience);
  ...
  const generatorHandler=createHttpHandler({jobs:generatorJobs,applySettings:generatorApply,validateInput:validateGeneratorInput,basePath:'/v1/generator',openapiPath:fileURLToPath(new URL('../generator/openapi.json',import.meta.url))});
