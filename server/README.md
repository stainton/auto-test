...
生产部署使用 `automation/`（默认 4501）：planner 和 generator 仍保持原有 HTTP 路径，但共享一个进程、浏览器依赖、按需求 ID 的探索经验，以及按目标系统 origin 自动隔离的产品级探索经验。探索经验是可选优化：产品级经验自动复用于同一目标系统，且会先做有界的在线校验；失效时只探索受影响页面并写回修复结果。需求级经验仍会随请求复用，generator 额外按需求带回 `explorationRecords`，并在一个批次内跨用例共享新发现。旧的 `planner/` 与 `generator/` 入口保留给兼容和单独开发调试。
