...
后续 generator、healer 分别增加 `server/generator/`、`server/healer/`，实现自己的契约、流程和入口，复用 shared/runtime；planner 仍只提供测试设计 HTTP 接口。构建和部署也分别放在对应的 `build/<service>` 与 `deploy/kubernetes/<service>`。
